...
let Bid = require('../models/bidModel')
    ...
    let checkNewItemId = setInterval(() => {
        if(message.type === 'noItem'){
            if(message.content.currentItem[0] !== undefined){
                (async ()=>{
                    let check = await Item.find({bidded: false}).sort({start_bid_date: 1}).limit(1);
                    if(check.length > 0){
                        pickItem();
                    }
               })();
...
function nextItem(){
    ...
    (async()=>{
        ...
        if(message.content.previousBidsUser.length > 0){
            ...
            await Item.updateOne({_id: message.content.currentItem[0]._id}, {bidded: true, sold: true});
            await Bid.create({
            ...
            await Item.updateOne({_id: message.content.currentItem[0]._id}, {bidded: true});
            pickItem();
        }
        message.content.previousBidsUser = [];
...
wss.on('connection', function connection(ws, req) {
        if(req.headers.cookie){
            ...
            (async ()=>{
                let checkUser = await Session.find({_id: userCookie});
                    ...
                    for(let i=0; i< checkUser.length; i++){
                        ...
                        if(expireDate > new Date().getTime()){
                            ...
                            if(JSON.parse(checkUser[i].session).passport){
                                ...
                                let user = await User.find({_id: userId}, {_id: 1, address: 1});
                                if(user.length > 0)
                                    ws.user = user;
                                else
                                    ws.user = undefined;
    ...
    (async ()=>{
        try {
            ...
            if(message.type !== 'noItem' && message.content.currentItem[0] !== undefined){
                ...
                let category = await Category.find({_id: currentItem[0].category_id});
                let owner = await User.find({_id: currentItem[0].user_id});
                ...
                res.json({
                    currentItem, previousBids, category, owner, currentPrice, previous_bids, startBidDate, loggedInUser, noItem
                })
            }
            else{
              const noItem = true;
